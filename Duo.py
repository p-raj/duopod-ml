...
def get_recognize(mapping_id, url, language_code, channel_id, episode_id, target_language, title):
    print(mapping_id, url, language_code, channel_id, episode_id, target_language, title)
    r = requests.get(url)
    ...
    with open(LANGUAGE_LIST_PATH) as json_file:
    ...
    r = requests.post(UPDATE_STATUS_URL.format(mapping_id), data=json.dumps(data))
    print(r.status_code, r.text)
    translate_text(mapping_id, filename, target_language, channel_id, episode_id, title)


def translate_text(mapping_id, transcript_file, target_language, channel_id, episode_id, title):
...
def text_to_speech(mapping_id, text_file, language_code, channel_id, episode_id):
    ...
    s3Resource.meta.client.upload_file(filename, 'duopod',
                                       ...
                                       ExtraArgs={'ACL': 'public-read'})
    s3_link = "https://duopod.s3.ap-south-1.amazonaws.com/{0}/{1}/{2}/podcast.txt".format(channel_id, episode_id,
                                                                                              language_code)
    data = {
        "status": "completed",
        "link": s3_link
...
def create_transcript():
    ...
    heavy_process = Process(target=get_recognize, daemon=True, args=(mapping_id, podcast_url, podcast_language, channel_id, episode_id, target_language, title))
    heavy_process.start()
